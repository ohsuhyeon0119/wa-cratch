...
3. **브랜치 + 워크스페이스 격리** (`superpowers:using-git-worktrees` 호출)
   - 이슈 제목/레이블을 보고 아래 규칙으로 prefix를 결정한다:
     | prefix | 판단 기준 |
     |--------|-----------|
     | `feat/` | 새 기능 추가 |
     | `fix/` | 버그 수정 |
     | `refactor/` | 기능 변경 없는 코드 개선 |
     | `chore/` | 빌드·의존성·CI 설정 등 유지보수 |
     | `docs/` | 문서 작업 |
     | `style/` | UI 스타일·레이아웃만 변경 |
     | `test/` | 테스트 코드만 추가/수정 |
     | `setting/` | 프로젝트 설정 파일 변경 |
   - 브랜치 이름 형식: `<prefix>/$ARGUMENTS-간단한설명` (예: `fix/17-sprite-overlap`)
   - 격리된 워크트리 생성
